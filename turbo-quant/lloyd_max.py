...
def compute_expected_distortion(d: int, bits: int, centroids, boundaries, use_exact: bool = False) -> float:
    pdf = (lambda x: beta_pdf(x, d)) if use_exact else (lambda x: gaussian_approx_pdf(x, d))
    sigma = 1.0 / math.sqrt(d)
...
class LloydMaxCodebook:
    """Precomputed Lloyd-Max codebook for a given dimension and bit-width (cached)."""
    ...
    def __init__(self, d: int, bits: int, use_exact: bool = False):
        ...
        _codebook_cache[(d, bits, use_exact)] = self
