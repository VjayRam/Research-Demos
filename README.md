...
**Benchmarks** (RTX 4070 Laptop GPU, AC power, 8 runs/config): the fused Triton decode kernel beats the native backend consistently, 1.16x-1.57x across every cache size/head dim tested — see [`rdkv/README.md`](rdkv/README.md#benchmarks) for the full tables, the allocation-vs-budget tradeoff, and a root-caused writeup of an earlier "regression" that turned out to be a battery-power measurement artifact, not a real kernel defect.
